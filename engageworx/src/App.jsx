...
const TENANTS = {
  serviceProvider: {
    ...
    logo: "EW",
    role: "superadmin",
    colors: { primary: "#00C9FF", accent: "#E040FB", bg: "#0D1117", surface: "#161B22", border: "#30363D", divider: "rgba(255,255,255,0.2)", text: "#F0F6FC", muted: "#ABB4BF" },
                ...
                {isConfiguring && (
                  <div style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${C.primary}33`, borderRadius: "0 0 12px 12px", borderTop: "none", padding: "20px 24px", marginTop: -1 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
                      ...
                      <div>
                        <div style={{ color: "rgba(255,255,255,0.4)", fontSize: 10, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 4, fontWeight: 700 }}>Primary Contact Email</div>
                        <input defaultValue={c.primary_contact_email || c.digest_email || ''} placeholder="billing@company.com" onBlur={async function(e) { var v = e.target.value.trim(); await supabase.from('tenants').update({ primary_contact_email: v || null }).eq('id', c.id); }} style={inputStyleTM} />
                        <div style={{ color: 'rgba(255,255,255,0.2)', fontSize: 9, marginTop: 3 }}>For billing notifications and escalation alerts. Use "Add Team Member" below to create logins.</div>
                      </div>
                      <div>
                    ...
                    <div style={{ marginBottom: 16, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                      ...
                      <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                        ...
                        <button onClick={async function() {
                          ...
                          try {
                            var ims = await supabase.auth.getSession();
                            var imjwt = ims.data.session ? ims.data.session.access_token : null;
                            var r = await fetch('/api/invite-member', {
                              method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + imjwt },
              ...
              <div>
                ...
                <Button type="button" variant="accent" disabled={loginLoading} onClick={async () => {
                  ...
                  try {
                    ...
                    if (checkoutData.url) {
                      // Notification handled by Stripe webhook + signup-notify
                    ...
                    // Queue signup error for platform visibility
                    fetch("/api/signup-notify", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ name: loginForm.fullName, email: loginForm.email, company: loginForm.companyName, plan: "starter", error: err.message || "Unknown" }),
                    }).catch(function() {});
