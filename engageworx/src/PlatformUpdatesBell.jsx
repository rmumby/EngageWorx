...
// Props:
//   userId     — the authenticated user's id (required for read-state tracking)
//   audience   — portal audience: 'sp' | 'csp' | 'master_agent' | 'agent' | 'tenant'
//                "all"-targeted updates are always visible.
export default function PlatformUpdatesBell({ userId, audience }) {
  ...
  return (
    <div style={{ position: 'relative', display: 'inline-block' }}>
      <button onClick={toggle} aria-label="Platform updates" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '6px 10px', color: '#fff', cursor: 'pointer', fontSize: 16, position: 'relative' }}>
        🔔
        {unreadCount > 0 && (
            ...
            {updates.length === 0 ? (
              ...
              <div style={{ display: 'grid', gap: 10 }}>
                {updates.map(function(u) {
                  var unread = !readIds.has(u.id);
                  return (
                    <div key={u.id} style={{ padding: 10, borderRadius: 8, background: unread ? 'rgba(224,64,251,0.08)' : 'rgba(255,255,255,0.02)', border: '1px solid ' + (unread ? 'rgba(224,64,251,0.3)' : 'rgba(255,255,255,0.06)') }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        {unread && <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#E040FB' }} />}
