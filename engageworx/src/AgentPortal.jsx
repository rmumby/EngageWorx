...
export default function AgentPortal({ agentTenantId, onLogout, onBack, profile, onOpenTenantPortal }) {
  ...
  useEffect(function() {
    ...
    (async function() {
      try {
        var r = await supabase.from('tenants').select('brand_primary, brand_secondary').eq('id', agentTenantId).maybeSingle();
        if (r.data) {
          var p = {};
  ...
  var labelStyle = { color: 'rgba(255,255,255,0.4)', fontSize: 11, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 6, fontWeight: 700 };
          ...
          <div>
            ...
            <div style={Object.assign({}, card, { marginBottom: 20 })}>
              ...
              <div style={{ display: 'grid', gap: 8 }}>
                {[
                  { month: 'March 2026', direct: directCommission * 0.9, override: overrideCommission * 0.8, status: 'Paid' },
                  { month: 'February 2026', direct: directCommission * 0.7, override: overrideCommission * 0.6, status: 'Paid' },
                  { month: 'January 2026', direct: directCommission * 0.5, override: overrideCommission * 0.4, status: 'Paid' },
                ].map(function(p, i) {
                  ...
                  return <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 120px 120px 100px 80px', alignItems: 'center', gap: 16, padding: '12px 18px', background: 'rgba(255,255,255,0.02)', borderRadius: 10, border: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <div style={{ textAlign: 'center' }}><div style={{ color: C.muted, fontSize: 10, marginBottom: 2 }}>DIRECT</div><div style={{ color: '#00E676', fontWeight: 700 }}>${p.direct.toFixed(0)}</div></div>
                    <div style={{ textAlign: 'center' }}><div style={{ color: C.muted, fontSize: 10, marginBottom: 2 }}>OVERRIDE</div><div style={{ color: C.primary, fontWeight: 700 }}>${p.override.toFixed(0)}</div></div>
                    <div style={{ textAlign: 'right' }}><div style={{ color: '#fff', fontWeight: 800, fontSize: 15 }}>${total.toFixed(0)}</div><div style={{ color: C.muted, fontSize: 10 }}>total</div></div>
                    <div style={{ textAlign: 'right' }}><span style={{ background: 'rgba(0,230,118,0.12)', border: '1px solid rgba(0,230,118,0.3)', borderRadius: 6, padding: '3px 8px', color: '#00E676', fontSize: 10, fontWeight: 700 }}>{p.status}</span></div>
                  </div>;
                })}
              </div>
            </div>
              ...
              <div style={{ display: 'grid', gap: 16 }}>
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <input value={agentBrand.websiteUrl || ''} onChange={function(e) { setAgentBrand(function(b) { return Object.assign({}, b, { websiteUrl: e.target.value }); }); }} placeholder="https://yourdomain.com" style={Object.assign({}, inputStyle, { flex: 1 })} />
                    <button onClick={detectAgentBrand} disabled={!agentBrand.websiteUrl || agentBrand.detecting} style={{ background: agentBrand.detecting ? 'rgba(255,255,255,0.06)' : 'linear-gradient(135deg, #FFD600, #FF6B35)', border: 'none', borderRadius: 8, padding: '10px 14px', color: agentBrand.detecting ? '#6B8BAE' : '#000', fontWeight: 700, cursor: agentBrand.websiteUrl && !agentBrand.detecting ? 'pointer' : 'not-allowed', fontSize: 13, whiteSpace: 'nowrap', fontFamily: "'DM Sans', sans-serif", opacity: !agentBrand.websiteUrl ? 0.5 : 1 }}>{agentBrand.detecting ? '⏳ Detecting…' : '✨ Auto-detect'}</button>
                  </div>
                  <div style={{ fontSize: 11, color: C.muted, marginTop: 4 }}>Auto-fills brand name, colors and logo from your website.</div>
                  {agentBrand.name && agentBrand.websiteUrl && (<div style={{ marginTop: 8, padding: '8px 12px', background: 'rgba(0,230,118,0.08)', border: '1px solid rgba(0,230,118,0.25)', borderRadius: 8, fontSize: 12, color: '#00E676', fontWeight: 600 }}>✅ Detected: {agentBrand.name}{agentBrand.primary ? ' · ' + agentBrand.primary : ''}</div>)}
                </div>
                <div>
                ...
                <button onClick={async function() {
                  var res = await supabase.from('tenants').update({ brand_name: agentBrand.name || (agentInfo ? agentInfo.name : ''), brand_primary: agentBrand.primary || '#FFD600', brand_secondary: agentBrand.accent || '#FF6B35', brand_logo_url: agentBrand.logoUrl || null, website_url: agentBrand.websiteUrl || null }).eq('id', agentTenantId);
                  if (res.error) { alert('Save failed: ' + res.error.message); } else { alert('Branding saved!'); }
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={function() { setShowSandbox(false); }}>
            <div style={{ background: C.surface, border: '1px solid ' + C.border, borderRadius: 16, padding: 32, width: 480, maxHeight: '80vh', overflowY: 'auto' }} onClick={function(e) { e.stopPropagation(); }}>
              ...
              {sandboxResult && sandboxResult.success ? (
                ...
                <div>
                  {sandboxResult && sandboxResult.error && (<div style={{ background: 'rgba(255,59,48,0.1)', border: '1px solid rgba(255,59,48,0.3)', borderRadius: 8, padding: '10px 14px', marginBottom: 16, color: '#FF3B30', fontSize: 13 }}>{sandboxResult.error}</div>)}
                  <div style={{ display: 'grid', gap: 14 }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                      <div><div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 6, fontWeight: 700 }}>Contact Name</div><input value={sandboxForm.fullName} onChange={function(e) { setSandboxForm(Object.assign({}, sandboxForm, { fullName: e.target.value })); }} placeholder="Jane Smith" style={inputStyle} /></div>
                      <div><div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 6, fontWeight: 700 }}>Company Name *</div><input value={sandboxForm.companyName} onChange={function(e) { var n = e.target.value; setSandboxForm(Object.assign({}, sandboxForm, { companyName: n, password: generatePassword(n) })); }} placeholder="Prospect Co" style={inputStyle} /></div>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={function() { setShowDemoForm(false); }}>
            <div style={{ background: C.surface, border: '1px solid ' + C.border, borderRadius: 16, padding: 32, width: 480, maxHeight: '80vh', overflowY: 'auto' }} onClick={function(e) { e.stopPropagation(); }}>
              ...
              {demoResult && demoResult.success ? (
                ...
                <div>
                  {demoResult && demoResult.error && (<div style={{ background: 'rgba(255,59,48,0.1)', border: '1px solid rgba(255,59,48,0.3)', borderRadius: 8, padding: '10px 14px', marginBottom: 16, color: '#FF3B30', fontSize: 13 }}>{demoResult.error}</div>)}
                  <div style={{ display: 'grid', gap: 14 }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                      <div><div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 6, fontWeight: 700 }}>Contact Name</div><input value={demoForm.fullName} onChange={function(e) { setDemoForm(Object.assign({}, demoForm, { fullName: e.target.value })); }} placeholder="Jane Smith" style={inputStyle} /></div>
                      <div><div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 6, fontWeight: 700 }}>Company Name *</div><input value={demoForm.companyName} onChange={function(e) { var n = e.target.value; setDemoForm(Object.assign({}, demoForm, { companyName: n, password: generatePassword(n) })); }} placeholder="Prospect Co" style={inputStyle} /></div>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
