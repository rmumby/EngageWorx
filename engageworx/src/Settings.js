...
const CHANNEL_DEFS = [
  ...
  { id: "voice", label: "Voice", icon: "📞", color: "#FFD600", fields: [
    ...
    { key: "voicemail_email", label: "Voicemail Email Recipients (extra)", placeholder: "email1@company.com, email2@company.com", hint: "Comma-separated. These recipients receive voicemails in addition to the digest email." },
...
if (!tenantId) {
  ...
  const aiAssistField = async (channelId, fieldKey, currentValue, aiContext, businessName) => { try { const _s = await supabase.auth.getSession(); const _tok = _s && _s.data && _s.data.session ? _s.data.session.access_token : ""; const res = await fetch("/api/ai-advisor", { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + _tok }, body: JSON.stringify({ max_tokens: 150, messages: [{ role: "user", content: `Improve this for ${businessName || "our business"}: "${currentValue}". Context: ${aiContext}. Return only the improved text.` }] }) }); const data = await res.json(); const improved = (data.content || []).find(b => b.type === "text")?.text || ""; if (improved) updateChannelField(channelId, fieldKey, improved.trim()); } catch (e) { alert("AI assist failed."); } };
  const aiTonePreview = async (tone, businessName, setPreview) => { try { const _s = await supabase.auth.getSession(); const _tok = _s && _s.data && _s.data.session ? _s.data.session.access_token : ""; const res = await fetch("/api/ai-advisor", { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + _tok }, body: JSON.stringify({ max_tokens: 200, messages: [{ role: "user", content: `Write a sample welcome email opening in this tone: "${tone || "warm and professional"}". Company: ${businessName || "our business"}. 2-3 sentences.` }] }) }); const data = await res.json(); const preview = (data.content || []).find(b => b.type === "text")?.text || ""; setPreview(preview); } catch (e) { alert("AI preview failed."); } };
              ...
              {[
                ...
                return (
                  <button key={lang.code} onClick={async function() {
                    ...
                    color: isActive ? C.primary : C.text, fontWeight: isActive ? 700 : 400,
      ...
      {activeTab === "webhooks" && (
        <div>
          ...
          {webhooksLoading ? <div style={{ textAlign: "center", padding: 40, color: C.muted }}>Loading webhooks...</div> : liveWebhooks.length === 0 ? (<div style={{ ...card, textAlign: "center", padding: 40 }}><div style={{ fontSize: 40, marginBottom: 12 }}>🔗</div><div style={{ color: C.text, fontWeight: 700, fontSize: 16, marginBottom: 6 }}>No webhooks configured</div><button onClick={() => setShowNewWebhook(true)} style={btnPrimary}>Create Your First Webhook</button></div>) : (
            <div style={{ display: "grid", gap: 10 }}>{liveWebhooks.map(wh => { const successRate = wh.total_deliveries > 0 ? Math.round((wh.successful_deliveries / wh.total_deliveries) * 1000) / 10 : null; const lastTriggered = wh.last_triggered_at ? new Date(wh.last_triggered_at).toLocaleString() : "Never"; const testStatus = webhookTestResult[wh.id]; return (<div key={wh.id} style={{ ...card, display: "grid", gridTemplateColumns: "1fr 120px 140px 80px auto", alignItems: "center", gap: 14, borderLeft: `4px solid ${wh.status === "active" ? "#00E676" : wh.status === "failed" ? "#FF3B30" : "#FFD600"}` }}><div><div style={{ color: C.text, fontWeight: 700, fontSize: 14 }}>{wh.name}</div><div style={{ fontFamily: "monospace", color: C.muted, fontSize: 11, marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{wh.url}</div><div style={{ display: "flex", gap: 4, marginTop: 6, flexWrap: "wrap" }}>{(wh.events || []).map(ev => <span key={ev} style={{ background: `${C.primary}12`, color: C.primary, borderRadius: 4, padding: "1px 6px", fontSize: 9, fontFamily: "monospace" }}>{ev}</span>)}</div></div><div>{successRate !== null ? (<><div style={{ color: successRate >= 99 ? "#00E676" : successRate >= 95 ? "#FFD600" : "#FF3B30", fontSize: 16, fontWeight: 700 }}>{successRate}%</div><div style={{ color: C.muted, fontSize: 10 }}>{wh.total_deliveries} deliveries</div></>) : <div style={{ color: C.muted, fontSize: 11 }}>No data yet</div>}</div><div style={{ color: C.muted, fontSize: 11 }}>{lastTriggered}</div><div><button onClick={() => toggleWebhookStatus(wh)} style={{ ...badge(wh.status === "active" ? "#00E676" : wh.status === "failed" ? "#FF3B30" : "#FFD600"), cursor: "pointer", border: "none", background: (wh.status === "active" ? "#00E676" : wh.status === "failed" ? "#FF3B30" : "#FFD600") + "18" }}>{wh.status}</button></div><div style={{ display: "flex", gap: 6 }}><button onClick={() => testWebhook(wh)} disabled={testStatus === "testing"} style={{ ...btnSec, padding: "6px 10px", fontSize: 11, color: testStatus === "success" ? "#00E676" : testStatus && testStatus.startsWith("failed") ? "#FF3B30" : C.text }}>{testStatus === "testing" ? "..." : testStatus === "success" ? "✓ OK" : testStatus ? "✗ Fail" : "Test"}</button><button onClick={() => { setEditingWebhook({ ...wh }); setShowNewWebhook(false); }} style={{ ...btnSec, padding: "6px 10px", fontSize: 11 }}>Edit</button><button onClick={() => deleteWebhook(wh.id)} style={{ ...btnSec, padding: "6px 10px", fontSize: 11, color: "#FF3B30" }}>✕</button></div></div>); })}</div>
          )}
        </div>
      )}
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {[
                ...
                <div key={opt.id} onClick={() => !staleModeSaving && saveStaleMode(opt.id)} style={{
                  ...
                  <div style={{ color: staleMode === opt.id ? C.primary : C.text, fontWeight: 700, fontSize: 13 }}>{opt.label}</div>
