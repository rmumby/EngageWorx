  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const { data: ls } = await supabase.from('leads').select('id, name, pipeline_stage_id, pipeline_stages(display_name), urgency, last_activity_at, value, estimated_value, deal_value').eq('company_id', selected.id);
          ...
          <div style={card}>
            ...
            {detail.leads.length === 0 ? <div style={{ color: C.muted, fontSize: 12 }}>No leads yet.</div> : detail.leads.map(l => (
              <div key={l.id} style={{ padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.04)', color: C.text, fontSize: 13 }}>
                ...
                <div style={{ color: C.muted, fontSize: 11 }}>{(l.pipeline_stages && l.pipeline_stages.display_name) || '—'} · {l.urgency || '—'}</div>
              </div>
            ))}
          </div>
        </div>
...
export default function ContactsModule({ C, tenants, viewLevel = "tenant", currentTenantId, demoMode = false, onNavigate }) {
          ...
          try {
            ...
            if (!leadId) {
              ...
              var newLead = await supabase.from('leads').insert({
                ...
                type: 'Direct Business', urgency: 'Warm', pipeline_stage_id: csvStageId, source: 'csv_import',
  ...
  async function convertToLead(contact) {
    ...
    try {
      if (contact.company) {
        var existing = await supabase.from('leads').select('id, name, pipeline_stages(display_name)').ilike('company', contact.company).eq('tenant_id', resolvedTenantId).limit(1).maybeSingle();
        if (existing.data) {
          setConvertResult({ exists: true, leadId: existing.data.id, stage: (existing.data.pipeline_stages && existing.data.pipeline_stages.display_name) || null });
      ...
      var ins = await supabase.from('leads').insert({
        ...
        pipeline_stage_id: convertStageId, urgency: 'Warm', type: 'Direct Business',
  ...
  const openMergeModal = () => {
    ...
    const primary = chosen.slice().sort((a, b) => {
      const ta = a.created ? (a.created.getTime ? a.created.getTime() : new Date(a.created).getTime()) : 0;
      const tb = b.created ? (b.created.getTime ? b.created.getTime() : new Date(b.created).getTime()) : 0;
      return ta - tb;
    })[0];
    ...
    fields.forEach(f => {
      ...
      initial[f] = (withValue || primary).id;
    });
                ...
                <div onClick={e => e.stopPropagation()} style={{ background: '#0d1425', border: '1px solid rgba(224,64,251,0.35)', borderRadius: 14, padding: 24, maxWidth: 900, width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
                  ...
                  <div style={{ marginBottom: 14 }}>
                    ...
                    <select value={mergePrimaryId || ''} onChange={e => setMergePrimaryId(e.target.value)} style={Object.assign({}, inputStyle, { width: '100%' })}>
                      {chosen.map(c => (
                        <option key={c.id} value={c.id}>{[c.firstName, c.lastName].filter(Boolean).join(' ') || c.email || c.phone} · created {c.created ? (c.created.toLocaleDateString ? c.created.toLocaleDateString() : String(c.created)) : '—'}</option>
                      ))}
                    </select>
                  </div>
