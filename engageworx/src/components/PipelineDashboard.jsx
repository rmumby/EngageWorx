...
function LeadCard({ lead, onSelect }) {
  ...
  return (
    <div onClick={() => onSelect(lead)}
      ...
      <div style={{ fontWeight: 700, fontSize: "14px", color: "var(--text-primary)", marginBottom: "2px", paddingRight: "80px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{lead.company || lead.name}</div>
      <div style={{ fontSize: "12px", color: "var(--text-muted)", marginBottom: "6px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{lead.name && lead.name !== lead.email && !lead.name.includes('@') ? lead.name : "—"}</div>
