// api/contacts.js — Contact operations (dedup, check, wedding couple)
...
// POST /api/contacts?action=mark-as-wedding-couple   — create wedding + partner + plan from contact
// POST /api/contacts?action=unmark-as-wedding-couple — unlink contact from wedding (keeps wedding row)
...
module.exports = async function handler(req, res) {
  ...
  // ── MARK-AS-WEDDING-COUPLE ─────────────────────────────────────────────
  if (action === 'mark-as-wedding-couple') {
    ...
    if (!body.partner_first_name) return res.status(400).json({ error: 'partner_first_name required' });
    if (!body.wedding_date) return res.status(400).json({ error: 'wedding_date required' });

    // Look up primary contact + verify tenant
    var { data: primary, error: primaryErr } = await supabase.from('contacts')
      .select('id, tenant_id, first_name, last_name, email, is_wedding_couple, wedding_id')
      .eq('id', contactId).maybeSingle();
    if (primaryErr || !primary) return res.status(404).json({ error: 'Contact not found' });
    var wTenantId = primary.tenant_id;
    ...
    if (!isSA) {
      var { data: mem } = await supabase.from('tenant_members').select('id').eq('tenant_id', wTenantId).eq('user_id', user.id).eq('status', 'active').maybeSingle();
      if (!mem) return res.status(403).json({ error: 'Not authorized for this tenant' });
    }

    // If already a wedding couple, return existing
    if (primary.is_wedding_couple && primary.wedding_id) {
      return res.status(200).json({ already_exists: true, wedding_id: primary.wedding_id });
    }

    try {
      // (a) Create partner contact
      var partnerPayload = {
        tenant_id: wTenantId,
        ...
        status: 'active',
        source: 'wedding_couple',
        is_wedding_couple: true,
      };
      var { data: partner, error: partnerErr } = await supabase.from('contacts')
        .insert(partnerPayload).select('id').single();
      if (partnerErr) throw new Error('Partner contact create failed: ' + partnerErr.message);

      // (b) Create wedding row
      var displayName = (primary.first_name || '') + ' & ' + (body.partner_first_name || '');
      var weddingStatus = body.status || 'planning';
      var { data: wedding, error: weddingErr } = await supabase.from('weddings').insert({
        tenant_id: wTenantId,
      ...
      if (weddingErr) throw new Error('Wedding create failed: ' + weddingErr.message);

      // (c) Create empty wedding_plans row
      ...
      if (planErr) console.warn('[contacts] wedding_plans insert error (non-fatal):', planErr.message);

      // (d) Update primary contact: link to wedding
      var { error: updateErr } = await supabase.from('contacts')
        .update({ is_wedding_couple: true, wedding_id: wedding.id })
        .eq('id', primary.id);
      if (updateErr) console.warn('[contacts] Primary contact update error:', updateErr.message);

      // (e) Update partner contact: link to wedding
      var { error: partnerUpdateErr } = await supabase.from('contacts')
        .update({ wedding_id: wedding.id })
        .eq('id', partner.id);
      if (partnerUpdateErr) console.warn('[contacts] Partner contact update error:', partnerUpdateErr.message);

      console.log('[contacts] Wedding couple created:', { wedding_id: wedding.id, primary: primary.id, partner: partner.id, tenant: wTenantId });
      ...
      console.error('[contacts] mark-as-wedding-couple error:', err.message);
      return res.status(500).json({ error: err.message });
    }
  }

  // ── UNMARK-AS-WEDDING-COUPLE ──────────────────────────────────────────
  if (action === 'unmark-as-wedding-couple') {
    var authHeader2 = req.headers.authorization || '';
    var token2 = authHeader2.replace('Bearer ', '');
    if (!token2) return res.status(401).json({ error: 'Missing auth token' });
    var { data: { user: user2 }, error: authErr2 } = await supabase.auth.getUser(token2);
    if (authErr2 || !user2) return res.status(401).json({ error: 'Invalid auth token' });

    var unmarkContactId = body.contact_id;
    if (!unmarkContactId) return res.status(400).json({ error: 'contact_id required' });

    var { data: contact, error: contactErr } = await supabase.from('contacts')
      .select('id, tenant_id, is_wedding_couple, wedding_id')
      .eq('id', unmarkContactId).maybeSingle();
    if (contactErr || !contact) return res.status(404).json({ error: 'Contact not found' });

    // Verify caller has access
    var { data: cp2 } = await supabase.from('user_profiles').select('role').eq('id', user2.id).maybeSingle();
    var isSA2 = cp2 && (cp2.role === 'superadmin' || cp2.role === 'super_admin' || cp2.role === 'sp_admin');
    if (!isSA2) {
      var { data: mem2 } = await supabase.from('tenant_members').select('id').eq('tenant_id', contact.tenant_id).eq('user_id', user2.id).eq('status', 'active').maybeSingle();
      if (!mem2) return res.status(403).json({ error: 'Not authorized for this tenant' });
    }

    // Unlink — do NOT delete the wedding row
    var { error: unlinkErr } = await supabase.from('contacts')
      .update({ is_wedding_couple: false, wedding_id: null })
      .eq('id', unmarkContactId);
    if (unlinkErr) return res.status(500).json({ error: unlinkErr.message });

    console.log('[contacts] Wedding couple unlinked:', { contact_id: unmarkContactId, tenant: contact.tenant_id });
