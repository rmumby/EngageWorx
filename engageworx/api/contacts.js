...
module.exports = async function handler(req, res) {
  ...
  if (action === 'mark-as-wedding-couple') {
    ...
    try {
      var { data: partner, error: partnerErr } = await supabase.from('contacts')
        .insert({ tenant_id: wTenantId, first_name: body.partner_first_name, last_name: body.partner_last_name || '', email: body.partner_email || null, status: 'active', source: 'wedding_couple', is_wedding_couple: true })
        .select('id').single();
      ...
      var { data: wedding, error: weddingErr } = await supabase.from('weddings').insert({
        tenant_id: wTenantId, display_name: displayName, wedding_date: body.wedding_date,
        status: body.status || 'planning', primary_contact_id: primary.id, partner_contact_id: partner.id,
      }).select('id').single();
      ...
      var { error: planErr } = await supabase.from('wedding_plans').insert({
        wedding_id: wedding.id, tenant_id: wTenantId, ceremony: {}, evening: {}, guests: {},
      });
      if (planErr) console.warn('[contacts] wedding_plans insert error (non-fatal):', planErr.message);

      await supabase.from('contacts').update({ is_wedding_couple: true, wedding_id: wedding.id }).eq('id', primary.id);
      await supabase.from('contacts').update({ wedding_id: wedding.id }).eq('id', partner.id);

      console.log('[contacts] Wedding couple created:', { wedding_id: wedding.id, primary: primary.id, partner: partner.id, tenant: wTenantId });
      return res.status(200).json({ success: true, wedding_id: wedding.id, partner_contact_id: partner.id, display_name: displayName });
    } catch (err) {
  ...
  if (action === 'unmark-as-wedding-couple') {
    ...
    var unmarkId = body.contact_id;
    if (!unmarkId) return res.status(400).json({ error: 'contact_id required' });

    var { data: contact, error: contactErr } = await supabase.from('contacts')
      .select('id, tenant_id').eq('id', unmarkId).maybeSingle();
    ...
    var { error: unlinkErr } = await supabase.from('contacts')
      .update({ is_wedding_couple: false, wedding_id: null }).eq('id', unmarkId);
    if (unlinkErr) return res.status(500).json({ error: unlinkErr.message });

    console.log('[contacts] Wedding couple unlinked:', { contact_id: unmarkId, tenant: contact.tenant_id });
