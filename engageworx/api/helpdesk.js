...
async function createTicket(req, res) {
  ...
  var { data: ticket, error } = await supabase
    ...
    .insert({
      ...
      submitter_email,
      submitter_user_id,
      submitter_tenant_id: submitter_tenant_id || tenant_id,
      subject,
      description,
  ...
  await supabase.from('ticket_messages').insert({
    ...
    author_user_id: submitter_user_id,
    content: description
  });
...
async function runAIResponse(ticket, latestMessage, history) {
  var { data: kbArticles } = await supabase
    ...
    .eq('published', true)
    .or(ticket.tenant_id ? 'tenant_id.is.null,tenant_id.eq.' + ticket.tenant_id : 'tenant_id.is.null')
    .limit(6);
...
async function getTicket(req, res) {
  ...
  var { data: ticket } = await supabase.from('support_tickets').select('*').eq('id', ticket_id).single();
...
async function escalateTicket(req, res) {
  ...
  await supabase.from('ticket_messages').insert({
    ...
    author_type: 'system',
    author_name: 'System',
    content: 'Ticket escalated to human agent. Reason: ' + reason,
    is_internal: true
  });
