...
- **Tenant-admin RLS: three failed attempts.** Adding policies that let tenant admins manage their own team has broken superadmin login each time. The subscription gate (`view = "no_tenant"`) fires because AuthContext queries `user_profiles` during login. The suspected culprit is the "Members read tenant teammate profiles" policy on `user_profiles` — it calls `get_user_tenant_ids()` which reads `tenant_members`, and something in that chain causes the `user_profiles` SELECT in `fetchProfile` (AuthContext.js line 25-29) to return empty for the superadmin user.
  ...
  **Alternative to investigate:** The superadmin's `user_profiles` row may not have a `tenant_members` row at all (superadmin might only exist in `user_profiles` with no tenant membership). If so, `get_user_tenant_ids(superadmin_id)` returns empty → the teammate policy returns no rows → BUT the existing `USING (id = auth.uid())` policy should still let the superadmin read their own profile (policies are OR'd). If this doesn't work, the issue is that adding ANY new policy somehow invalidates the existing one — which would indicate a Supabase RLS evaluation bug or a policy name collision.
