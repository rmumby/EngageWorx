-- ROLLBACK for 20260609_060_sp_admin_rls_carveout.sql
-- Restores the pre-carve-out tenant RLS policies by stripping the SP-admin OR carve-out from each clause (deterministic transform of the forward migration).
-- MANUAL ROLLBACK ARTIFACT -- not part of the normal forward sequence; apply only to revert.
...
CREATE POLICY kb_articles_access ON public.helpdesk_kb_articles
  ...
  USING (((tenant_id IS NULL) OR (tenant_id IN ( SELECT tenant_members.tenant_id
   FROM tenant_members
  WHERE (tenant_members.user_id = auth.uid())))))
  WITH CHECK (((tenant_id IS NULL) OR (tenant_id IN ( SELECT tenant_members.tenant_id
   FROM tenant_members
  WHERE (tenant_members.user_id = auth.uid())))));
...
CREATE POLICY "Admins can modify own branding" ON public.tenant_branding
  FOR ALL TO public
  USING (((tenant_id)::uuid IN ( SELECT tenant_members.tenant_id
   FROM tenant_members
  WHERE ((tenant_members.user_id = auth.uid()) AND (tenant_members.status = 'active'::text) AND (tenant_members.role = ANY (ARRAY['admin'::text, 'owner'::text, 'manager'::text]))))))
  WITH CHECK (((tenant_id)::uuid IN ( SELECT tenant_members.tenant_id
   FROM tenant_members
  WHERE ((tenant_members.user_id = auth.uid()) AND (tenant_members.status = 'active'::text) AND (tenant_members.role = ANY (ARRAY['admin'::text, 'owner'::text, 'manager'::text]))))));
...
CREATE POLICY "Users can read own row or tenant peers" ON public.users
  FOR SELECT TO public
  USING (((id = auth.uid()) OR (tenant_id IN ( SELECT tenant_members.tenant_id
   FROM tenant_members
  WHERE ((tenant_members.user_id = auth.uid()) AND (tenant_members.status = 'active'::text))))));
